...
contract('HashedTimelock swap between two ERC721 tokens', (accounts) => {
  ...
  before(async () => {
    ...
    await assertTokenBal(AliceERC721, Alice, 2, 'balance not transferred to Alice in before()');
    await assertTokenBal(AliceERC721, Bob, 0, 'Bob should not have any AliceERC721 tokens in before()');
    await assertTokenBal(BobERC721, Bob, 2, 'balance not transferred to Bob in before()');
    await assertTokenBal(BobERC721, Alice, 0, 'Alice should not have any BobERC721 tokens in before()');
  ...
  it('Step 1: Alice sets up a swap with Bob in the AliceERC721 contract', async () => {
    timeLock2Sec = (await helpers.time.latest()) + 10;
    const newSwapTx = await newSwap(AliceERC721, 1 /*token id*/, htlc, { hashlock: hashPair.hash, timelock: timeLock2Sec }, Alice, Bob);
  ...
  it('Step 2: Bob sets up a swap with Alice in the BobERC721 contract', async () => {
    ...
    timeLock2Sec = (await helpers.time.latest()) + 10;
    const newSwapTx = await newSwap(BobERC721, 1 /*token id*/, htlc, { hashlock: hashPair.hash, timelock: timeLock2Sec }, Bob, Alice);
  ...
  it('Step 3: Alice as the initiator withdraws from the BobERC721 with the secret', async () => {
    ...
    await assertTokenBal(BobERC721, Alice, 1, `Alice should now own 1 Bob token`);
  ...
  it("Step 4: Bob as the counterparty withdraws from the AliceERC721 with the secret learned from Alice's withdrawal", async () => {
    ...
    await assertTokenBal(AliceERC721, Bob, 1, `Bob should now own 1 Alice token`);
  ...
  describe('Test the refund scenario:', () => {
    it('the swap is set up with 10 sec timeout on both sides', async () => {
      timeLock2Sec = (await helpers.time.latest()) + 10;
      let newSwapTx = await newSwap(AliceERC721, 2 /*token id*/, htlc, { hashlock: hashPair.hash, timelock: timeLock2Sec }, Alice, Bob);
      a2bSwapId = txContractId(newSwapTx);

      newSwapTx = await newSwap(BobERC721, 2 /*token id*/, htlc, { hashlock: hashPair.hash, timelock: timeLock2Sec }, Bob, Alice);
      b2aSwapId = txContractId(newSwapTx);
      ...
      // advance time by 100 seconds to make the timelock expire
  ...
  const newSwap = async (token, tokenId, htlc, config, initiator, counterparty) => {
    ...
    return htlc.newContract(counterparty, config.hashlock, config.timelock, token.address, tokenId, {
      from: initiator,
    });
  };
});
