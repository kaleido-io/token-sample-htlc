...
contract('HashedTimelock swap between ERC721 token and ERC20 token (Delivery vs. Payment)', (accounts) => {
  ...
  before(async () => {
    ...
    await assertTokenBal(CommodityTokens, Alice, 2, 'Alice should own two Commodity tokens');
    await assertTokenBal(CommodityTokens, Bob, 0, 'Bob should not have any Commodity tokens in before()');
    await assertTokenBal(PaymentTokens, Bob, 1000, 'balance not transferred to Bob in before()');
    await assertTokenBal(PaymentTokens, Alice, 0, 'Alice should not have any payment tokens in before()');
