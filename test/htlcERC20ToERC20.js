...
contract('HashedTimelock swap between two ERC20 tokens', (accounts) => {
  ...
  before(async () => {
    ...
    await assertTokenBal(AliceERC20, Alice, senderInitialBalance, 'balance not transferred to Alice in before()');
    await assertTokenBal(AliceERC20, Bob, 0, 'Bob should not have any AliceERC20 tokens in before()');
    await assertTokenBal(BobERC20, Bob, senderInitialBalance, 'balance not transferred to Bob in before()');
    await assertTokenBal(BobERC20, Alice, 0, 'Alice should not have any BobERC20 tokens in before()');
  ...
  it('Step 1: Alice sets up a swap with Bob in the AliceERC20 contract', async () => {
    timeLockSeconds = (await helpers.time.latest()) + 10;
  ...
  it('Step 3: Alice as the initiator withdraws from the BobERC20 with the secret', async () => {
    ...
    await assertTokenBal(BobERC20, Alice, tokenAmount, `Alice doesn't not own ${tokenAmount} tokens`);
  ...
  it("Step 4: Bob as the counterparty withdraws from the AliceERC20 with the secret learned from Alice's withdrawal", async () => {
    ...
    await assertTokenBal(AliceERC20, Bob, tokenAmount, `Bob doesn't not own ${tokenAmount} tokens`);
  ...
  describe('Test the refund scenario:', () => {
    ...
    it('the swap is set up with 5sec timeout on both sides', async () => {
      ...
      // increase the block time by 100 seconds to make the timeout expire
  ...
  const newSwap = async (token, htlc, config, initiator, counterparty) => {
    ...
    return htlc.newContract(counterparty, config.hashlock, config.timelock, token.address, tokenAmount, {
      from: initiator,
    });
  };
});
